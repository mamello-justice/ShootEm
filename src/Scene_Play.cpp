...
void Scene_Play::sMovement() {
...

	{
		...
		if (cInput.up and !cInput.down) {
			cTransform.velocity.y = -1;
		}
		else if (cInput.down and !cInput.up) {
			cTransform.velocity.y = 1;
		}
		else {
		...
		if (cInput.left and !cInput.right) {
			cTransform.velocity.x = -1;
		}
		else if (cInput.right and !cInput.left) {
			cTransform.velocity.x = 1;
		}
		else {
...
void Scene_Play::sLifespan() {
	if (m_paused or !m_isLifespanActive) { return; }
